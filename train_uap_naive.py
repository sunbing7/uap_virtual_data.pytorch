...
from attack.universal_pert import universal_perturbation
...
from utils.custom_loss import (LogitLoss, BoundedLogitLoss, NegativeCrossEntropy, BoundedLogitLossFixedRef,
                               BoundedLogitLoss_neg)
...
def parse_arguments():
    ...
    parser.add_argument('--loss_function', default='bounded_logit_fixed_ref', choices=['ce', 'neg_ce',
                                                                                       'logit', 'bounded_logit',
...
def main():
    ...
    uap = universal_perturbation(data_train_loader,
                                 target_network,
                                 num_classes=num_classes,
    ...
    np.save(uap_path + '/uap_' + str(args.target_class) + adaptive + '.npy', uap.cpu().detach().numpy())
    plt.savefig(model_path + '/uap_' + str(args.target_class) + adaptive + '.png')
    plt.show()
    print('uap saved!')

    uap = torch.from_numpy(uap.cpu().detach().numpy() / np.array(std).reshape(1, 3, 1, 1))
    test_sr, nt_sr, clean_test_acc, _test_sr, _nt_sr = eval_uap(pretrained_data_test_loader, target_network, uap,
