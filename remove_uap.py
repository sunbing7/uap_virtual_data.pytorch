...
from utils.network import get_network, set_parameter_requires_grad, set_parameter_requires_grad_selected
from utils.network import get_num_parameters, get_num_non_trainable_parameters, get_num_trainable_parameters
from utils.training import train, train_repair, save_checkpoint, metrics_evaluate, reconstruct_model_repair
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Remove UAP')
    ...
    parser.add_argument('--split_layer', type=str, default='classifier.4',
                        help='trainable layer (default: classifier.4)')
...
def main():
    ...
    #update the network
    #target_network, num_classes = reconstruct_model_repair(target_network, args.pretrained_arch, neu_idx, split_layer=args.split_layer)
    ...
    set_parameter_requires_grad_selected(target_network, args.split_layer, requires_grad=False)
    ...
    # Set the target model into training mode
    ...
    optimizer = torch.optim.Adam(target_network.parameters(), lr=state['learning_rate'])
    #'''
    # Measure the time needed for the UAP generation
    start = time.time()
    train_repair(data_loader=data_train_loader, uap=uap,
